...
final class ManifestProviderTests: XCTestCase {
    ...
    func testPollSendsEndpointHeadersAndBody() async {
        let client = StubManifestHTTPClient(responses: [
            .success(#"{ "ok": true }"#)
        ])
        ...
        XCTAssertEqual(client.requests, [
            ManifestHTTPRequest(
                ...
                headers: ["Authorization": "Bearer static-token"],
                body: Data(#"{"query":"status"}"#.utf8)
            )
        ])
        XCTAssertEqual(snapshot.health, .ok)
        XCTAssertEqual(snapshot.metrics, [
            Metric(id: "ok", label: "OK", value: .bool(true))
        ])
    }
    ...
    func testExecutesManifestCommandEndpointWithHeadersAndBodyArguments() async throws {
        let client = StubManifestHTTPClient(responses: [
            .success(#"{ "ok": true }"#)
        ])
        let manifest = ProviderManifest(
            ...
            commands: [
                ProviderManifest.Command(
                    ...
                    endpoint: ProviderManifest.Endpoint(
                        ...
                        body: #"{"target":"{target}","enabled":"{enabled}"}"#
                    )
                )
            ]
        )
        ...
        XCTAssertEqual(client.requests, [
            ManifestHTTPRequest(
                method: .post,
                url: URL(string: "https://example.test/output")!,
                headers: ["Content-Type": "application/json"],
                body: Data(#"{"target":"ac%2Foutlet","enabled":"true"}"#.utf8)
            )
        ])
    }
