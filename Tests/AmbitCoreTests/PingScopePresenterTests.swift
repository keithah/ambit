...
final class PingScopePresenterTests: XCTestCase {
    ...
    func testFormatRoundsAndHandlesMissing() {
        XCTAssertEqual(PingScopePresenter.format(ms: 14.6), "15ms")
        XCTAssertEqual(PingScopePresenter.format(ms: nil), "--ms")
    }

    func testReadoutHealthyWithinFreshness() {
        let r = PingScopePresenter.readout(latest: sample(6, agoSeconds: 20), health: .healthy, now: now, freshness: 60)
    ...
    func testReadoutAgesOutStaleLatest() {
        let r = PingScopePresenter.readout(latest: sample(6, agoSeconds: 90), health: .healthy, now: now, freshness: 60)
    ...
    func testReadoutNoDataWhenNoSample() {
        let r = PingScopePresenter.readout(latest: nil, health: .noData, now: now, freshness: 60)
        XCTAssertEqual(r.text, "--ms")
        XCTAssertEqual(r.statusLabel, "No Data")
    }

    func testGlyphStacksTextAndTone() {
        let g = PingScopePresenter.glyph(latest: sample(22, agoSeconds: 5), health: .healthy, now: now, freshness: 60)
    ...
    func testNiceMaxRoundsUpToReadableMaximum() {
        XCTAssertEqual(PingScopePresenter.niceMax([14, 78, 107]), 125)
        XCTAssertEqual(PingScopePresenter.ticks(max: 125), [125, 62.5, 0])
        XCTAssertEqual(PingScopePresenter.niceMax([3, 5, 8]), 25)   // floor for tiny series
    }
    ...
    func testWindowedFiltersToRangeAndSorts() {
        ...
        let windowed = PingScopePresenter.windowed(samples, range: .fiveMinutes, now: now)
