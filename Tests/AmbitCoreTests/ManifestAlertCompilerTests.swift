...
final class ManifestAlertCompilerTests: XCTestCase {
    ...
    func testCompiledManifestRuleEmitsProviderMetricTarget() async throws {
        let manifest = ProviderManifest(
            schemaVersion: 1,
            id: "demo.alerts",
            displayName: "Alerts Demo",
        ...
        let engine = AlertEngine(rules: ManifestAlertCompiler.rules(from: manifest))
        let snapshot = EngineSnapshot(providers: [
            ProviderInstanceID(rawValue: "demo.alerts"): SourceState(value: ProviderSnapshot(health: .ok, metrics: [
                Metric(id: "battery_percent", label: "Battery", value: .percent(12))
            ]))
        ])

        let events = await engine.evaluate(snapshot, now: Date(timeIntervalSince1970: 0))

        XCTAssertEqual(events.map(\.ruleID), ["demo.alerts.battery.low"])
        XCTAssertEqual(events.first?.target, .providerMetric(providerID: "demo.alerts", metricID: "battery_percent"))
    }
}
