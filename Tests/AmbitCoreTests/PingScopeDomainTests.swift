...
final class PingScopeDomainTests: XCTestCase {
    func testValidationRequiresUsableNameAndAddress() {
        let host = PingScopeHostConfig(displayName: "  ", address: "  ", method: .icmp)
    ...
    func testTCPAndUDPRequirePortButICMPDoesNot() {
        XCTAssertTrue(PingScopeHostConfig(displayName: "a", address: "h", method: .tcp, port: nil).validationErrors.contains(.invalidPort))
        XCTAssertTrue(PingScopeHostConfig(displayName: "a", address: "h", method: .udp, port: nil).validationErrors.contains(.invalidPort))
        XCTAssertFalse(PingScopeHostConfig(displayName: "a", address: "h", method: .icmp, port: nil).validationErrors.contains(.invalidPort))
    }

    func testApplyingMethodSetsMethodAwarePort() {
        var host = PingScopeHostConfig(displayName: "a", address: "h", method: .tcp, port: 443)
    ...
    func testTimeoutProbeReturnsProbeResultWhenProbeWins() async {
        ...
        let host = PingScopeHostConfig(displayName: "a", address: "h", method: .tcp, port: 443, timeout: 1)
    ...
    func testTimeoutProbeReturnsTimeoutWhenProbeIsLate() async {
        ...
        let host = PingScopeHostConfig(displayName: "a", address: "h", method: .tcp, port: 443, timeout: 0.25)
        let result = await probe.measure(host)
